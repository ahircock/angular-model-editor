...
export class ModelDataService {
  ...
  async createTemplate(): Promise<ModelData> {
    ...
    const newModelId = await this.dbConnectService.getNextId('M');
    ...
    const newModel = await this.convertDBToModelData( newModelDB );
  ...
  async updateModel( updateModel: ModelData ): Promise<ModelData> {
    ...
    const updateDBModel = await this.dbConnectService.updateModel( this.convertModelDataToDB(updateModel) );

    // update the record in the cache
    const newUpdateModel = await this.convertDBToModelData(updateDBModel);
    const findModelIndex: number = this.modelCache.findIndex( element => element._id === newUpdateModel._id );
  ...
  async cloneModel( clonedModel: ModelData, cloneModelFromTemplate = false ): Promise<ModelData> {
    ...
    const newDBModel = await this.dbConnectService.createModel(this.convertModelDataToDB(newModel));
  ...
  async deleteModel( deleteModel: ModelData ): Promise<void> {
    ...
    const modelIndex: number = this.modelCache.findIndex( element => element._id === deleteModel._id );
  ...
  async addAction( model: ModelData, action: ActionData ): Promise<ModelData> {
    ...
    const newAction: ModelActionData = {
    ...
    for ( const actionRule of action.specialRules ) {
      const newRule: SpecialRuleData = {
        _id: actionRule._id,
  ...
  private async calculateModelStats( model: ModelData ): Promise<ModelData> {
    ...
    for ( const action of model.actions ) {
      ...
      const baseAction = await this.actionDataService.getActionById( action._id );
      action.HIT = baseAction.HIT;
      action.DMG = baseAction.DMG;
    }

    // add the special rule costs
    for ( const specialRule of model.specialRules ) {
  ...
  private convertModelDataToDB( modelData: ModelData ): ModelDBData {
    ...
    for ( const rule of modelData.specialRules ) {
      modelDBData.specialRuleIds.push( rule._id );
    }

    // initialize actions
    for ( const action of modelData.actions ) {
      const modelActionDBData: ModelActionDBData = {
  ...
  private async ruleUpdated( updatedRule: SpecialRuleData) {
    ...
    for ( let model of this.modelCache ) {
      ...
      const ruleIndex = model.specialRules.findIndex( element => element._id === updatedRule._id );
      if ( ruleIndex >= 0 ) {
  ...
  private async actionUpdated( updatedAction: ActionData ) {
    ...
    for ( let model of this.modelCache ) {
      ...
      const actionIndex = model.actions.findIndex( element => element._id === updatedAction._id );
      if ( actionIndex >= 0 ) {
        const modelAction: ModelActionData =
          Object.assign( { modelActionName: model.actions[actionIndex].modelActionName }, updatedAction );
